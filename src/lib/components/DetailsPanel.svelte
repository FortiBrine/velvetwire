<script lang="ts">
	...
	const timeModes: { value: TimeMode; label: string }[] = [
		...
		{ value: 'frozen', label: $i18n.sections.timestamp.modes.frozen },
		{ value: 'fixed', label: $i18n.sections.timestamp.modes.fixed }
	];
</script>
...
<div class="flex flex-col gap-4 p-5 overflow-y-auto h-full">
	...
	<div class="space-y-3">
		...
		{#if activity.timeMode === 'frozen'}
			<div class="grid grid-cols-2 gap-2">
				<div class="space-y-1.5">
					<Label class="text-xs text-muted-foreground">{$i18n.sections.timestamp.frozen.hours}</Label>
					<Input
						type="number"
						min="0"
						max="99"
						bind:value={activity.frozenHours}
						class="h-8 text-xs bg-input border-border text-center"
					/>
				</div>
				<div class="space-y-1.5">
					<Label class="text-xs text-muted-foreground">{$i18n.sections.timestamp.frozen.minutes}</Label>
					<Input
						type="number"
						min="0"
						max="59"
						bind:value={activity.frozenMinutes}
						class="h-8 text-xs bg-input border-border text-center"
					/>
				</div>
			</div>
			<p class="text-xs text-muted-foreground">{$i18n.sections.timestamp.frozen.info}</p>
		{:else if activity.timeMode === 'fixed'}
			<div class="space-y-1.5">
				<Label class="text-xs text-muted-foreground">{$i18n.sections.timestamp.fixed.label}</Label>
				<Input
					type="datetime-local"
					bind:value={activity.fixedDatetime}
					class="h-8 text-xs bg-input border-border"
				/>
			</div>
			<p class="text-xs text-muted-foreground">{$i18n.sections.timestamp.fixed.info}</p>
